...
def get_gas_price(chainid: int = 1) -> dict:
    ...
    try:
        ...
        # Chuyển đổi an toàn sang float trước khi ép kiểu int
        safe = int(float(result.get("SafeGasPrice", 0)))
        fast = int(float(result.get("FastGasPrice", 0)))
        propose = int(float(result.get("ProposeGasPrice", 0)))
        ...
        return {
            ...
            "summary": f"ETH Gas: Safe={safe} Gwei | Propose={propose} Gwei | Fast={fast} Gwei"
        }
