...
# 1. Tinh chỉnh Hình 1 V5: Xóa bỏ artifact và nới lề dưới
img_source = Image.open('documents/split_pages/page_14.jpg')
w, h = img_source.size

# Cắt vùng an toàn nhưng đủ rộng để không mất quân cờ
# ymin: 580 (đủ cao để lấy Tướng đen), xmax: 480 (tránh chữ 例)
left, top, right, bottom = 135 * w / 1000, 580 * h / 1000, 480 * w / 1000, 860 * h / 1000
crop = img_source.crop((left, top, right, bottom))

cw, ch = crop.size
draw_crop = ImageDraw.Draw(crop)

# Xóa bỏ các artifact nhỏ ở góc trên bên phải (nơi Nguyên khoanh vùng)
# Dùng một dải trắng nhỏ để quét sạch lề phải và lề trên
draw_crop.rectangle([cw * 0.9, 0, cw, ch], fill='white') # Lề phải
draw_crop.rectangle([0, 0, cw, ch * 0.05], fill='white') # Lề trên

# Tạo canvas mới cao hơn để nới lề dưới rộng ra thêm
padding_bottom = 80
hinh_1_final_w = cw
...
# Đặt nhãn (Hình 1) ở vị trí thấp hơn, thoáng hơn
draw_final_hinh.text((hinh_1_final_w/2, hinh_1_final_h - 40), "(Hình 1)", fill='black', font=font_label, anchor='mm')
hinh_1_canvas.save('documents/split_pages/hinh_1_v5_clean.jpg')

# 2. Render trang hoàn chỉnh V5
...
# Chèn bàn cờ V5
h1v5 = Image.open('documents/split_pages/hinh_1_v5_clean.jpg')
h1w, h1h = h1v5.size
disp_w = 800
disp_h = int(h1h * (disp_w / h1w))
canvas.paste(h1v5.resize((disp_w, disp_h), Image.Resampling.LANCZOS), (200, y_offset))
...
canvas.save('documents/Tuong_Ky_Trang_1_Hoan_Hao_V5.jpg')
print('Render V5 complete.')
